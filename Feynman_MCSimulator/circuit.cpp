    ...
    for (int l=0 ; l<csize->num_layers ; l++) {
        ...
        if (layer->num_type_gates[1] > 0) {
            ...
            for (int g=0 ; g<layer->num_type_gates[1] ; g++) {
                ...
                g1p1_ptr->fdata.param = (float) aux.param;
                float *MR_aux_ptr = (float *)g1p1_ptr->fdata.m;
                float *aux_ptr = (float *)aux.m;
                for (int x=0 ; x<8 ; x++, MR_aux_ptr++, aux_ptr++) {
                    *MR_aux_ptr = (float) *aux_ptr;
                }
        ...
        TGate2P1 * g2p1_ptr = (TGate2P1 *) layer->gates[3];
            for (int g=0 ; g<layer->num_type_gates[3] ; g++) {
                ...
                g2p1_ptr->fdata.param = (float) aux.param;
                float *MR_aux_ptr = (float *)g2p1_ptr->fdata.m;
                float *aux_ptr = (float *)aux.m;
