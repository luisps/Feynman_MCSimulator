...
void layer_w (TCircuitLayer *layer, int l,
    ...
    for (int g=0 ; g<layer->num_type_gates[0] ; g++) {
        ...
        const int curr_state_qb = current_state->qb_value(qb_nbr);
#ifdef DEBUG_LAYER
        fprintf (stderr, "\tcurr: qb_nbr = %d -> qb=%d\n",qb_nbr, curr_state_qb);
#endif
        
        const int next_state_qb = next_state->qb_value(qb_nbr);
#ifdef DEBUG_LAYER
    ...
    for (int g=0 ; g<layer->num_type_gates[2] ; g++) {
        ...
        const int curr_state_qbs = current_state->qb_value(c_qb_nbr)*2+current_state->qb_value(t_qb_nbr);
        const int next_state_qbs = next_state->qb_value(c_qb_nbr)*2+next_state->qb_value(t_qb_nbr);
...
myReal layer_sample (TCircuitLayer* layer, int l, CState const * const current_state,
    ...
    //CState lnext_state(current_state.get_nqbs());
    ...
    for (int g=0 ; g<layer->num_type_gates[0] ; g++) {
        ...
        switch (gate->name) {
            case 0:             // id
                ...
                gatepdf = gate_id_sample (curr_state_qb, next_state_qb, gatewR, gatewI);
                break;
            case 1:             // h
                rnd = d(e);  // generate random nbr in [0, 1[
            ...
            case 2:             // x
                ...
                gatepdf = gate_x_sample (curr_state_qb, next_state_qb, gatewR, gatewI);
                break;
            case 3:             // y  -- NON-SYMMETRICAL !!!!!
                if (forwardSample)
                    gatepdf = gate_y_sample (curr_state_qb, next_state_qb, gatewR, gatewI);
                else
                    gatepdf = gate_y_sample_back (curr_state_qb, next_state_qb, gatewR, gatewI);
                break;
            case 4:             // z
                //gatepdf = gate_z_sample (curr_state_qb, rnd, next_state_qb, gatewR, gatewI);
                gatepdf = gate_z_sample (curr_state_qb, next_state_qb, gatewR, gatewI);
                break;
            case 5:             // s
//                gatepdf = gate_s_sample (curr_state_qb, rnd, next_state_qb, gatewR, gatewI);
                gatepdf = gate_s_sample (curr_state_qb, next_state_qb, gatewR, gatewI);
                break;
            case 6:             // t
//                gatepdf = gate_t_sample (curr_state_qb, rnd, next_state_qb, gatewR, gatewI);
                gatepdf = gate_t_sample (curr_state_qb, next_state_qb, gatewR, gatewI);
                break;
            default:             // id
                //gatepdf = gate_id_sample (curr_state_qb, rnd, next_state_qb, gatewR, gatewI);
                gatepdf = gate_id_sample (curr_state_qb, next_state_qb, gatewR, gatewI);
        ...
        next_state->qb_set_value(qb_nbr, next_state_qb);
    ...
    for (int g=0 ; g<layer->num_type_gates[2] ; g++) {
        ...
        switch (gate->name) {
            case 20:             // id2
                ...
                gatepdf = gate_id2_sample (curr_state_qbs, next_state_qbs, gatewR, gatewI);
                break;
            case 21:             // cx
                //gatepdf = gate_cx_sample (curr_state_qbs, rnd, next_state_qbs, gatewR, gatewI);
                gatepdf = gate_cx_sample (curr_state_qbs, next_state_qbs, gatewR, gatewI);
                break;
            case 22:             // cz
                //gatepdf = gate_cz_sample (curr_state_qbs, rnd, next_state_qbs, gatewR, gatewI);
                gatepdf = gate_cz_sample (curr_state_qbs, next_state_qbs, gatewR, gatewI);
                break;
            default:             // id2
                //gatepdf = gate_id2_sample (curr_state_qbs, rnd, next_state_qbs, gatewR, gatewI);
                gatepdf = gate_id2_sample (curr_state_qbs, next_state_qbs, gatewR, gatewI);
        ...
        // Set the appropriate bit in lnext_state
        next_state->qb_set_value(t_qb_nbr, next_state_qbs & 0x01);
        next_state->qb_set_value(c_qb_nbr, (next_state_qbs & 0x02) >> 1);
...
//                gatepdf = gate_t_sample (curr_state_qb, rnd, next_state_qb, gatewR, gatewI);
        ...
        // Set the appropriate bit in lnext_state
        if (next_state_qb==0) qb_reset(qb_nbr, lnext_state);
        else qb_set(qb_nbr, lnext_state);
    ...
    for (int g=0 ; g<layer->num_type_gates[2] ; g++) {
        ...
        // Set the appropriate bits in lnext_state
        if ((next_state_qbs & 0x01)==0) qb_reset(t_qb_nbr, lnext_state);
        else qb_set(t_qb_nbr, lnext_state);

        if ((next_state_qbs & 0x02)==0) qb_reset(c_qb_nbr, lnext_state);
        else qb_set(c_qb_nbr, lnext_state);
