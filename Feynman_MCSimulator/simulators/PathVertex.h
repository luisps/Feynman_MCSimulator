...
public:
    ...
    int append (void) {  // append to the last one (filling from the beginning)
        if (head >= nElements) return -1;
        head++;
        return head-1;
    }
    int prepend (void) {  // prepend to the first one (filling from the end)
        if (tail < 0) return -1;
        tail--;
        return tail+1;
    }
    void clear (void) {        
        head = 0;
        tail = nElements -1;
        //for (int i=0 ; i<nElements ; i++) data[i].state->reset_data();
    }
};
